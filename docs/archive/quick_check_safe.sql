...
SELECT
  ...
  CASE
    WHEN COUNT(*) = 0 THEN NULL
    ELSE ROUND((COUNT(*) FILTER (WHERE error_message LIKE '%429%')::NUMERIC / COUNT(*)::NUMERIC) * 100, 2)
  END as rate_limit_error_percent
...
SELECT
  ...
  remaining,
  CASE
    WHEN limit_total = 0 THEN NULL
    ELSE ROUND((remaining::NUMERIC / limit_total::NUMERIC) * 100, 2)
  END as remaining_percent,
  reset_at,
  CASE
    WHEN remaining = 0 THEN '🚨 制限到達'
    WHEN limit_total = 0 THEN '⚪ データなし'
...
SELECT
  ...
  CASE
    ...
    WHEN rs.rate_errors > 0 AND (rs.rate_errors::NUMERIC / rs.total_exec) > 0.1 THEN '🚨 429エラー頻発 (' || rs.rate_errors || '/' || rs.total_exec || ')'
...
SELECT
  ...
  COUNT(*) FILTER (WHERE executed_at > NOW() - INTERVAL '1 hour') as last_1h,
